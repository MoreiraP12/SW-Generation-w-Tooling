...
# Define retryable exceptions for Vertex AI
RETRYABLE_VERTEX_EXCEPTIONS = (Exception,)  # Generic for now, can be refined later
...
# --- New function for Vertex AI model ---
@retry_with_exponential_backoff(retryable_exceptions=RETRYABLE_VERTEX_EXCEPTIONS)
def call_vertex_gemma_api(prompt: str, project_id: str, endpoint_id: str, location: str, task_type: str):
    """
    Calls Vertex AI endpoint with retry logic handled by the decorator.
    Formats the prompt based on task type.
    """
    raw_response_text = "N/A"
    
    if not project_id or not endpoint_id or not aiplatform:
        print("--- Skipping Vertex AI API Call (Missing configuration) ---")
        return None, "Skipped: Missing configuration"
    
    # Adapt system prompt based on task type
    if task_type == "mcqa":
        system_prompt = "Please answer the following multiple-choice question. Respond with ONLY the single letter (A, B, C, or D) corresponding to the best answer."
    elif task_type == "yesno":
        system_prompt = "Based on the provided context and question, please answer with ONLY one word: Yes, No, or Maybe."
    ...
    try:
        ...
        # Create the instance payload - format according to your endpoint's requirements
        instance = {
            "prompt": full_prompt,
            "max_tokens": 15,
            "temperature": 0.1
        ...
        # Extract the response from the prediction
        if prediction and prediction.predictions:
            for pred in prediction.predictions:
                if isinstance(pred, dict) and "text" in pred:
                    raw_response_text = pred["text"]
                    # Parse the response based on task type
                    parsed_choice = parse_llm_response(raw_response_text, task_type)
                    return parsed_choice, raw_response_text
                elif isinstance(pred, str):
                    raw_response_text = pred
                    # Parse the response based on task type
                    parsed_choice = parse_llm_response(raw_response_text, task_type)
                    return parsed_choice, raw_response_text
            
            # If we get here, we couldn't find a suitable response format
            raw_response_text = f"API Error: Unexpected response format from Vertex AI. Response: {prediction.predictions}"
            print(f"  -> {raw_response_text}")
            return None, raw_response_text
        else:
            raw_response_text = f"API Error: Empty prediction response from Vertex AI."
            print(f"  -> {raw_response_text}")
            return None, raw_response_text
            
    except Exception as e:
        # Catch any errors during API interaction
        raw_response_text = f"Vertex AI Error: {e}"
...
# --- Main Evaluation Logic (Modified for Iterative Saving and Model Selection) ---
def evaluate_questions(
    ...
    Args:
        ...
        output_paths: Dictionary of output file paths for each model
        model_instances: Dictionary of model instances/configs to use
    ...
    enabled_models = {k: v for k, v in model_instances.items() if v['enabled']}
    
    if not enabled_models:
        print("No models enabled for evaluation. Exiting.")
        return

    task_type = config["task_type"]
    processed_count = 0
    failure_counts = {model: 0 for model in enabled_models}

    print(f"\nStarting evaluation for dataset '{args.dataset}' (Task Type: {task_type}).")
    print(f"Processing up to {max_questions or 'all'} questions...")
    print(f"Models enabled for evaluation: {', '.join(enabled_models.keys())}")
    ...
    # Define column orders for each model
    column_orders = {}
    for model in enabled_models:
        column_orders[model] = base_columns + [f'{model.lower()}_model', f'{model.lower()}_answer', f'{model.lower()}_raw_response', f'{model.lower()}_status']
    ...
    for i, question_data in enumerate(dataset):
        ...
        # --- Call APIs for each enabled model ---
        model_results = {}
        
        # Process Gemini if enabled
        if 'Gemini' in enabled_models and enabled_models['Gemini']['instance']:
            print("--- Calling Gemini API ---")
            gemini_answer, gemini_raw_response = call_gemini_api(
                prompt, 
                enabled_models['Gemini']['instance'], 
                task_type
            )
            model_results['Gemini'] = {
                'answer': gemini_answer,
                'raw_response': gemini_raw_response,
                'model_id': enabled_models['Gemini']['instance'].model_name
            }

        # Process Gemma (OpenRouter) if enabled
        if 'Gemma' in enabled_models and enabled_models['Gemma']['api_key'] and enabled_models['Gemma']['model_id']:
            print("--- Calling OpenRouter API (Gemma) ---")
            gemma_answer, gemma_raw_response = call_openrouter_gemma_api(
                prompt, 
                enabled_models['Gemma']['api_key'], 
                enabled_models['Gemma']['model_id'], 
                task_type
            )
            model_results['Gemma'] = {
                'answer': gemma_answer,
                'raw_response': gemma_raw_response,
                'model_id': enabled_models['Gemma']['model_id']
            }

        # Process NVIDIA if enabled
        if 'NVIDIA' in enabled_models and enabled_models['NVIDIA']['client'] and enabled_models['NVIDIA']['model_id']:
            print("--- Calling NVIDIA API (DeepSeek) ---")
            nvidia_answer, nvidia_raw_response = call_nvidia_api(
                prompt, 
                enabled_models['NVIDIA']['client'], 
                enabled_models['NVIDIA']['model_id'], 
                task_type
            )
            model_results['NVIDIA'] = {
                'answer': nvidia_answer,
                'raw_response': nvidia_raw_response,
                'model_id': enabled_models['NVIDIA']['model_id']
            }
            
        # Process Vertex if enabled
        if 'Vertex' in enabled_models and all([
            enabled_models['Vertex']['project_id'],
            enabled_models['Vertex']['endpoint_id'],
            enabled_models['Vertex']['location']
        ]):
            print("--- Calling Vertex AI API (Gemma) ---")
            vertex_answer, vertex_raw_response = call_vertex_gemma_api(
                prompt,
                enabled_models['Vertex']['project_id'],
                enabled_models['Vertex']['endpoint_id'],
                enabled_models['Vertex']['location'],
            ...
            model_results['Vertex'] = {
                'answer': vertex_answer,
                'raw_response': vertex_raw_response,
                'model_id': f"Vertex AI - {enabled_models['Vertex']['endpoint_id']}"
            }
        ...
        # Determine status for each model and log results
        for model_name, result in model_results.items():
            answer = result['answer']
            status = "API Error/Invalid Response"
            
            if answer is not None:
                status = "Correct" if str(answer) == str(correct_answer) else "Incorrect"
                
            print(f"{model_name} Result: {answer} ({status})")
            
            # Check if failed (Incorrect or Error)
            failed = status != "Correct"
            ...
            if failed:
                ...
                model_info = {
                    **base_info,
                    f'{model_name.lower()}_model': result['model_id'],
                    f'{model_name.lower()}_answer': answer if answer is not None else "N/A",
                    f'{model_name.lower()}_raw_response': result['raw_response'] or "N/A",
                    f'{model_name.lower()}_status': status,
                }
                
                # Save the failure data
                save_failure_iteratively(
                    model_info, 
                    output_paths[model_name], 
                    column_orders[model_name],
                    model_name
                )

        # Check if all models were correct
        if all(status != "Correct" for model_name, result in model_results.items() for status in 
               ["Correct" if result['answer'] is not None and str(result['answer']) == str(correct_answer) else "Incorrect"]):
    ...
    for model_name, count in failure_counts.items():
        print(f"Total {model_name} failures/errors saved to '{output_paths.get(model_name, 'N/A')}': {count}")
...
if __name__ == "__main__":
    ...
    # --- Argument Parsing ---
    parser = argparse.ArgumentParser(description="Evaluate LLMs on medical datasets with exponential backoff.")
    ...
    parser.add_argument("--models", "-m", type=str, default="all",
                        help="Comma-separated list of models to evaluate: gemini,gemma,nvidia,vertex (default: all)")
    
    # Add Vertex AI specific arguments
    parser.add_argument("--vertex_project", type=str, default=None,
                        help="Google Cloud Project ID for Vertex AI")
    parser.add_argument("--vertex_endpoint", type=str, default=None,
                        help="Endpoint ID for Vertex AI Gemma model")
    parser.add_argument("--vertex_location", type=str, default="us-central1",
                        help="Location for Vertex AI endpoint (default: us-central1)")
    
    args = parser.parse_args()

    # Parse the models argument
    selected_models = args.models.lower().split(",") if args.models.lower() != "all" else ["gemini", "gemma", "nvidia", "vertex"]
    
    # Determine output file names based on dataset
    dataset_key = args.dataset
    output_paths = {
        'Gemini': f"gemini_failed_{dataset_key}.csv",
        'Gemma': f"gemma_failed_{dataset_key}.csv",
        'NVIDIA': f"nvidia_deepseek_failed_{dataset_key}.csv",
        'Vertex': f"vertex_gemma_failed_{dataset_key}.csv"
    }

    # --- Initialize API Clients and Model Configurations ---
    
    # Gemini setup
    gemini_model_instance = None
    if "gemini" in selected_models and GOOGLE_API_KEY and genai:
        try:
    ...
    elif "nvidia" in selected_models and not NVIDIA_API_KEY:
        print("Skipping NVIDIA client initialization (NVIDIA_API_KEY not found).")
    elif "nvidia" in selected_models:
        print("Skipping NVIDIA client initialization (openai library failed to import).")
    
    # Vertex AI setup
    vertex_project_id = args.vertex_project or VERTEX_PROJECT_ID
    vertex_endpoint_id = args.vertex_endpoint or GEMMA_VERTEX_ENDPOINT_ID
    vertex_location = args.vertex_location or VERTEX_LOCATION
    
    if "vertex" in selected_models:
        if not vertex_project_id or not vertex_endpoint_id:
            print("Skipping Vertex AI setup (missing project ID or endpoint ID).")
            print("You can provide these via environment variables or command line arguments:")
            print("  --vertex_project PROJECT_ID --vertex_endpoint ENDPOINT_ID")
        elif not aiplatform:
            print("Skipping Vertex AI setup (google-cloud-aiplatform library failed to import).")
        else:
            print(f"Vertex AI configuration: Project={vertex_project_id}, Endpoint={vertex_endpoint_id}, Location={vertex_location}")

    # Prepare model instances for evaluation
    model_instances = {
        'Gemini': {
            'enabled': "gemini" in selected_models,
            'instance': gemini_model_instance
        },
        'Gemma': {
            'enabled': "gemma" in selected_models,
            'api_key': OPENROUTER_API_KEY,
            'model_id': GEMMA_MODEL_ID
        },
        'NVIDIA': {
            'enabled': "nvidia" in selected_models,
            'client': nvidia_client_instance,
            'model_id': NVIDIA_MODEL_ID
        },
        'Vertex': {
            'enabled': "vertex" in selected_models,
    ...
    if not enabled_models:
        print("Error: No models enabled for evaluation. Please specify at least one valid model. Exiting.")
        exit()
    else:
        models_status = ", ".join([f"{model}{'✓' if data.get('instance') or (data.get('api_key') and data.get('model_id')) or (model == 'Vertex' and data.get('project_id') and data.get('endpoint_id')) else '✗'}" 
                                  for model, data in enabled_models.items()])
        print(f"Models enabled for evaluation: {models_status}")
