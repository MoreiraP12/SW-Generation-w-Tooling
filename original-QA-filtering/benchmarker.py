        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(MAX_RETRIES + 1): # +1 to allow the initial try
                try:
                    ...
                    print(f"  -> Retryable error in {func.__name__}: {error_msg}. Retrying in {wait_time:.2f} seconds (Attempt {attempt + 1}/{MAX_RETRIES})...")
...
def load_data(config_key: str):
    ...
    try:
        ...
        if "BuilderConfig" in str(e) and "not found" in str(e):
             print(f"  -> HINT: Check if the config name '{config['hf_config']}' is correct. Available configs might be different (e.g., 'pqa_labeled' instead of 'pqa_l').")
        # Add hint for split name error
        if "Unknown split" in str(e):
             print(f"  -> HINT: Check if the split name '{config['split']}' is correct for this dataset/config.")
        return None, None

# <<< MODIFIED: Returns extracted raw_question_text >>>
def format_question_for_llm(question_data, config):
    """Formats the prompt and returns extracted question text."""
    task_type = config["task_type"]
    question_field = config["question_field"] # Uses corrected 'question' key for PubMedQA
    ...
    # Add context if available (Handles potential PubMedQA nesting)
    if config["context_field"]:
        # Use the key specified in config, but check for nesting if it's PubMedQA
        context_config_key = config["context_field"] # e.g., "CONTEXTS"
        context_data = question_data.get(context_config_key)

        # Check for the nested structure found in 'pqa_artificial' and 'pqa_labeled'
        ...
        # Fallback for original config structure (if CONTEXTS was top-level list/string)
        elif isinstance(context_data, list):
            context = "\n".join(filter(None, context_data)) # Filter None context parts
        elif isinstance(context_data, str):
            context = context_data # Single string context
        else:
            context = None # Ensure context is None if no valid data found

        if context:
             # Simple truncation if context is too long
             max_context_len = 3000
             if len(context) > max_context_len: context = context[:max_context_len] + "... (truncated)"
             prompt += f"Context:\n{context}\n\n"

    # Add question (using the extracted text)
    prompt += f"Question: {raw_question_text}\n\n" # Will be "Question: N/A" if missing

    # Add options for MCQA tasks (Unchanged logic from original)
    if task_type == "mcqa":
        prompt += "Options:\n"
        ...
        if isinstance(options_fields_config, list):
            if len(options_fields_config) >= 4:
                # Correctly get options based on field names in the list
                options = {
                ...
                for key, value in options.items():
                    prompt += f"{key}. {value}\n"
            else: prompt += "(Error: 'options_fields' list in config has fewer than 4 elements)\n"
        elif isinstance(options_fields_config, str):
            options_data = question_data.get(options_fields_config) # Get list using the key
            if isinstance(options_data, list) and len(options_data) >= 4:
                 options = {k: v for k, v in zip('ABCD', options_data)}
                 for key, value in options.items():
                     prompt += f"{key}. {value}\n"
            else: prompt += f"(Error: Could not format options - expected a list in field '{options_fields_config}' or list has < 4 items)\n"
        elif options_fields_config is None: prompt += "(Error: 'options_fields' not defined in config for MCQA task)\n"
        else: prompt += "(Error: Invalid 'options_fields' configuration type)\n"

    return prompt, options, raw_question_text # Return prompt, options, AND extracted question text
...
def get_ground_truth_answer(question_data, config):
    ...
    ground_truth_raw = question_data.get(answer_field) # Renamed variable
    ...
    if task_type == "mcqa":
        ...
        try: answer_index = int(ground_truth_raw)
        except (ValueError, TypeError): return f"Invalid Raw GT: {ground_truth_raw}"

        # <<< CORRECTED MedMCQA mapping to 0-based based on exploration >>>
        if config["hf_path"] == "medmcqa": # MedMCQA: 0-based index (cop)
        ...
        return str(ground_truth_raw).lower() # Ensure lowercase yes/no/maybe
    else:
        return str(ground_truth_raw)
...
def parse_llm_response(response_text: str, task_type: str):
    ...
    if task_type == "yesno":
        ...
        match_bracket = re.search(r'\s*\[\s*(yes|no|maybe)\s*\]', response_clean, re.IGNORECASE) # Added whitespace allowance
...
def call_gemini_api(prompt: str, model: genai.GenerativeModel, task_type: str):
    ...
    if task_type == "mcqa": system_instruction = "Respond ONLY with the single letter (A, B, C, or D)."
    elif task_type == "yesno": system_instruction = "Respond ONLY with one word: Yes, No, or Maybe."
    else: system_instruction = "Please answer the question."
    ...
    try:
        ...
        response = model.generate_content([system_instruction, prompt], generation_config=generation_config, request_options={'timeout': 60}) # Added timeout
...
def call_openrouter_gemma_api(prompt: str, api_key: str, model_id: str, task_type: str):
    ...
    if task_type == "mcqa": system_prompt = "Respond ONLY with the single letter (A, B, C, or D)."
    elif task_type == "yesno": system_prompt = "Respond ONLY with one word: Yes, No, or Maybe."
    else: system_prompt = "Please answer the question."

    payload = {"model": model_id, "messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": prompt}], "max_tokens": 15, "temperature": 0.1}
    ...
    try:
        ...
        if response_data.get("choices") and len(response_data["choices"]) > 0:
            ...
            if content is not None:
                ...
                if not raw_response_text: raw_response_text = "Empty Response"
                else: print(f"  -> OpenRouter Raw: '{raw_response_text[:100]}...'") # Log raw response
                parsed_choice = parse_llm_response(raw_response_text, task_type) # Attempt parsing
            else:
        ...
        if completion.choices and len(completion.choices) > 0:
            ...
            if message and message.content is not None:
                ...
                if not raw_response_text:
                     ...
                     print(f"  -> NVIDIA Raw (Finish: {finish_reason}): '{raw_response_text[:100]}...'") # Log raw response and finish reason
                ...
                if expected_pattern:
                    ...
                    if matches:
                        ...
                        if parsed_choice: print(f"  -> NVIDIA Parsed (Fallback): {parsed_choice}") # Log if fallback succeeded
                else: # No specific format expected
                     parsed_choice = parse_llm_response(raw_response_text, task_type)
                     if parsed_choice: print(f"  -> NVIDIA Parsed (General): {parsed_choice}") # Log if general parsing succeeded
                ...
                if parsed_choice is None and "Empty Response" not in raw_response_text:
                     ...
                     if finish_reason == 'length': print("  -> Warning: NVIDIA response likely truncated (max_tokens reached).")
